...
import { OpenConversationModel, CreateConversationModel } from '../models'
...
class ConversationService {
    ...
    private async createConversation(): Promise<void> {
        const conversationModel: OpenConversationModel = {
            participants: [
                {
        ...
        const conversationCreated = (
            await this._hejto.send(
                'conversation', {}, { method: 'POST', data: conversationModel }
            )
        )
        .headers
        .location

        this._conversationUuid = parseUuid(conversationCreated, 1)
    }
}
