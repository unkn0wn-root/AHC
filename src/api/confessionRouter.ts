...
async function getConfessionMiddleware(req: RequestWithConfession, res: Response, next: NextFunction) {
		try {
			...
			if (!confession)
				return res.status(404).json(makeAPIResponse(res, null, { message: 'confession not found' }))

			req.confession = confession
			return next()
		} catch(err) {
			logger.error('GET - Confession Middleware Error', { errMsg: err?.message, stack: err.stack })
			return res.status(500)
				.send(makeAPIResponse(
					res, null, { message: 'Sorry, Internal Server Error occured' }
					)
			)
		}
}
...
confessionRouter.get('/', async (req: RequestWithUser, res) => {
	try {
		...
		logger.error('GET - Confession Page Error', { errMsg: err?.message, stack: err.stack })
		return res.status(500)
				.send(makeAPIResponse(res, null, { message: 'Something went wrong when getting confession' }))
	}
})

confessionRouter.get('/confession/:id',
	accessMiddlewareV2('viewDetails'),
	getConfessionMiddleware, async (req: RequestWithConfession, res) => {
		try {
			...
			logger.error('GET - Confession Error', { errMsg: err?.message, stack: err.stack })
			return res.status(500)
				.send(makeAPIResponse(
					res, null, { message: 'Sorry, we have some rusty servers right now...' }
					)
			)
		}
	}
...
confessionRouter.delete('/confession/:id',
	...
	getConfessionMiddleware, async (req: RequestWithConfession, res) => {
		try {
			...
			logger.error('DELETE - Confession Error', { errMsg: err?.message, stack: err.stack })
			return res.status(500)
				.send(makeAPIResponse(
					res, null, { message: 'Confession was not deleted' }
					)
			)
		}
	}
...
confessionRouter.post('/confession/:id/accept',
	...
	async (req: RequestWithConfession<AcceptConfessionOptions>, res) => {
		...
		try {
			...
			logger.error('POST - Confession Error', { errMsg: err?.message, stack: err.stack })
			return res.status(500)
				.send(makeAPIResponse(
					res, null, { message: 'Sorry, we have some rusty servers right now...' }
					)
...
confessionRouter.put('/confession/:id/status',
	...
	async (req: RequestWithConfession, res) => {
		...
		try {
			...
			logger.error('PUT - Confession Status Error', { errMsg: err?.message, stack: err.stack })
			return res.status(500)
				.send(makeAPIResponse(
					res, null, { message: 'Status was not updated' }
					)
			)
		}
	}
...
confessionRouter.put('/confession/:id/tags',
	...
	async (req: RequestWithConfession & {body: {tag: string, tagValue: boolean}}, res) => {
		...
		try {
			...
			logger.error('PUT - Confession Tags Error', { errMsg: err?.message, stack: err.stack })
			return res.status(500)
				.send(makeAPIResponse(
					res, null, { message: 'Could not create tags...' }
					)
			)
		}
	}
...
confessionRouter.get('/confession/:id/ip',
	...
	async (req: RequestWithConfession, res) => {
		try {
			...
			logger.error('GET - Confession IP Error', { errMsg: err?.message, stack: err.stack })
			return res.status(500)
				.send(makeAPIResponse(
					res, null, { message: 'IP could not be retrieved. Server error' }
					)
			)
		}
	}
...
confessionRouter.get('/confession/:id/otherFromIp',
	...
	async (req: RequestWithConfession, res) => {
		try {
			...
			logger.error('GET - Confession OtherFromIP Error', { errMsg: err?.message, stack: err.stack })
			return res.status(500)
				.send(makeAPIResponse(
					res, null, { message: 'IP could not be retrieved. Server error' }
					)
